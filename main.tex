...
\subsection*{Methods for Using WOA as an Optimal Feature Selector} {
    Sayed et al. as well as Emary and Zawbaa use the WOA in a wrapper-based method with no adjustments to the original definition of WOA presented in 2016.
    Sayed et al. apply the algorithm for in a data classification setting where machine learning techniques are used to diagnos breast cancer, while Emary and Zawbaa determine the efficacy of WOA on several test data sets in the wrapper-based feature selector application.
    ...
    Zawbaa and Emary use the k-nearest neighbor (KNN) classifier in every test, and directly compare both the convergence rate and accuracy of WOA to feature selecting algorithms currently used in the field (2017).
    Mafarja and Mirjalili explore a hybrid meta-heuristic that combines exploration phase of WOA with the exploitation phase of Simulated Annealing (SA) as a feature selector on various test data sets (2017).
